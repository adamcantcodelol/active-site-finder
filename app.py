...
st.set_page_config(page_title="MBRC Active Site Finder", page_icon="🧬", layout="wide", initial_sidebar_state="collapsed")
...
if run_clicked:
    ...
    try:
        ...
        status_box = st.empty()
        fs.poll_until_complete(ticket_id, max_wait_seconds=300, on_status=lambda status, elapsed: status_box.info(f"Foldseek search: {status.lower()} — {elapsed}s elapsed"))
        status_box.empty()
        ...
        table_df=pd.DataFrame([{"Rank":i+1,"PDB / Chain":h.target_id,"Description":h.description or "n/a","RMSD (Å)":f"{h.rmsd:.2f}" if h.rmsd is not None else "n/a","TM-Score":f"{h.tm_score:.3f}" if h.tm_score is not None else "n/a","Seq. Identity":f"{h.seq_identity*100:.1f}%" if h.seq_identity is not None else "n/a"} for i,h in enumerate(table_hits)])
        ...
        if not predicted:
            ...
            site_df=pd.DataFrame([{"Residue":f"{r.query_resname or '?'}{r.display_resnum}","Query position":r.display_resnum,"Homologs agreeing":r.support_count,"Supporting structures":", ".join(r.supporting_hits[:6])+(" ..." if len(r.supporting_hits)>6 else "")} for r in shown])
