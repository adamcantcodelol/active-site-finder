...
.viewer-card{border:1px solid #dbe3ee;border-radius:12px;padding:10px;background:#fff}.viewer-title{font-weight:700;margin:2px 0 8px}
</style>
...
def map_site(hit,query_pdb,query_chain):
    ...
    try:
        ...
        return [SitePair(
            homolog_chain=p.get("tchain",hit.chain_id or "?"), homolog_resnum=p["tn"], homolog_insertion=p.get("ticode","") or "",
            homolog_resname=p["tname"], query_chain=p.get("qchain",query_chain or "?"), query_resnum=p["qn"],
            query_insertion=p.get("qicode","") or "", query_resname=p["qname"], exact=bool(p.get("exact")),
        ) for p in mapped]
    except Exception:
        return []
...
def render_sprite(hit,pairs,query_id):
    ...
    if len(chosen)!=3:
        st.info("This homolog does not have a validated local three-residue experimental site that can be mapped to the original protein. Try another homolog."); return None
    rows=[]
    for p in chosen:
        mark=" ✓" if p.exact else ""
        rows.append(f'<tr><td>{html.escape(p.homolog_chain)}{p.homolog_resnum}{html.escape(p.homolog_insertion)} {html.escape(p.homolog_resname)}</td><td class="sprite-arrow">matches</td><td class="sprite-exact">{html.escape(p.query_chain)}{p.query_resnum}{html.escape(p.query_insertion)} {html.escape(p.query_resname)}{mark}</td></tr>')
    ...
    homolog_cmd=chimera_select("#2",chosen[0].homolog_chain,[p.homolog_resnum for p in chosen]); original_cmd=chimera_select("#1",chosen[0].query_chain,[p.query_resnum for p in chosen])
    a,b=st.columns(2)
...
def render_viewer(pid,title,chain=None,residues=None,height=560,homolog_pid=None,homolog_chain=None,homolog_residues=None,mode=1):
    try:
        markup=viewer_html(pid,chain=chain,residues=residues,height=height,homolog_pdb_id=homolog_pid,homolog_chain=homolog_chain,homolog_residues=homolog_residues)
        if mode in (2,3,4):
            # The viewer initializes asynchronously; apply the requested mode after
            # its models have loaded. The iframe is recreated on Streamlit reruns.
            markup=markup.replace("init();",f"init(); setTimeout(()=>{{if(window.applyMode) window.applyMode({mode});}},1800);")
    except ValueError as exc:
        st.error(f"3D viewer configuration error: {exc}"); return
    st.markdown(f'<div class="viewer-card"><div class="viewer-title">{html.escape(title)}</div>',unsafe_allow_html=True)
    components.html(markup,height=height+52,scrolling=False)
...
if run:
    ...
    try:
        ...
        st.session_state.update(query_id=pdb_id,query_pdb=query_pdb,query_chain=query_chain,hits=hits,rmsd_hits=rmsd_hits); st.session_state["selected_hit_index"]=0; st.session_state["site_map_cache"]={}; st.session_state.pop("homolog_selector",None)
...
if "rmsd_hits" in st.session_state:
    ...
    with c3:
        tm=f"{best.tm_score:.3f}" if best.tm_score is not None else "n/a"; st.markdown(f'<div class="metric-card"><div class="metric-label">TM-score</div><div class="metric-value">{tm}</div><div class="metric-note">structural similarity</div></div>',unsafe_allow_html=True)
    with c4:
        ident=f"{best.seq_identity*100:.1f}%" if best.seq_identity is not None else "n/a"; st.markdown(f'<div class="metric-card"><div class="metric-label">Sequence identity</div><div class="metric-value">{ident}</div><div class="metric-note">aligned residues</div></div>',unsafe_allow_html=True)
    ...
    if not available_hits:
        st.info("None of the returned structural matches have a validated three-residue experimental site that can be mapped to the original protein."); chosen=None; selected_hit=None
    else:
        labels=[f"{rmsd_hits.index(h)+1}. {h.target_id} · chain {h.chain_id or '?'} · RMSD {h.rmsd:.2f} Å" for h in available_hits]
        previous=st.session_state.get("selected_hit_target"); default_index=next((i for i,h in enumerate(available_hits) if h.target_id==previous),0)
        selected_label=st.selectbox("Choose the similar protein",labels,index=default_index,key="homolog_selector"); selected_index=labels.index(selected_label); selected_hit=available_hits[selected_index]; st.session_state["selected_hit_target"]=selected_hit.target_id
        selected_pairs=cached_map_site(selected_hit,query_pdb,query_chain); chosen=render_sprite(selected_hit,selected_pairs,query_id)
    ...
    if predicted:
        ...
        pred_cmd=chimera_select("#1",query_chain,[r.query_resnum for r in site]); reveal_button("ChimeraX: select predicted active site",pred_cmd,"predicted_site",f"Open the original protein as model #1. This selects the predicted residues on chain {query_chain}.")
    ...
    chosen_homolog_residues=[p.homolog_resnum for p in chosen] if chosen else []; chosen_query_residues=[p.query_resnum for p in chosen] if chosen else []
    homolog_id=selected_hit.pdb_id if selected_hit else None
    if homolog_id and chosen:
        mode_labels={1:"1 — Full proteins + highlighted SPRITE residues",2:"2 — SPRITE residues only",3:"3 — Overlay full proteins + highlighted residues",4:"4 — Overlay SPRITE residues only"}
        mode=st.radio("3D view mode",list(mode_labels.keys()),format_func=lambda x:mode_labels[x],horizontal=False,key="viewer_mode",help="Modes 3 and 4 align the homolog to the original using the three matched active-site residues.")
        st.caption("Original active-site residues are orange; homolog active-site residues are cyan.")
        ...
        st.info("Select a SPRITE-compatible structural homolog to view the four comparison modes.")
