...
PDB SITE residues are preferred; when a PDB has no SITE records, this module
falls back to residues making atomic contact with a non-water HETATM ligand.
Those template residues are mapped through the Foldseek alignment onto the
query structure.
"""
...
def parse_site_residues(pdb_text: str, chain_id: Optional[str] = None) -> list[dict]:
    ...
    # SITE residue slots start at columns 19, 30, 41, 52 (1-indexed).
...
def parse_ca_residues(pdb_text: str, chain_id: Optional[str] = None) -> list[dict]:
    ...
    for line in pdb_text.splitlines():
        ...
        if key in seen:
            continue
        seen.add(key)
        residues.append({"resnum": resnum, "insertion_code": insertion, "resname": line[17:20].strip()})
    return residues
...
def _parse_atom(line: str) -> Optional[dict]:
    if len(line) < 54:
        return None
    try:
        return {"atom": line[12:16].strip(), "resname": line[17:20].strip(), "chain": line[21].strip(), "resnum": int(line[22:26]), "icode": line[26].strip(), "x": float(line[30:38]), "y": float(line[38:46]), "z": float(line[46:54])}
    except (TypeError, ValueError):
        return None


def parse_ligand_contact_residues(pdb_text: str, chain_id: Optional[str] = None, cutoff_angstrom: float = 4.0) -> list[dict]:
    """Fallback active-site annotation from contacts to non-solvent ligands."""
...
def _template_site_residues(pdb_text: str, chain_id: Optional[str]) -> list[dict]:
    ...
    return site if site else parse_ligand_contact_residues(pdb_text, chain_id)
...
def _map_binding_site_to_query(hit: Hit, target_structure: str, query_structure: str, query_chain_id: Optional[str]) -> list[tuple[int, str, str]]:
    ...
    q_index, t_index = hit.q_start - 1, hit.t_start - 1
    mapped: list[tuple[int, str, str]] = []
    for q_char, t_char in zip(hit.q_aln, hit.t_aln):
        q_present, t_present = q_char != "-", t_char != "-"
...
def predict_active_site(hits: list[Hit], query_pdb_text: str, query_chain_id: Optional[str] = None, top_n_hits: int = 15) -> list[ActiveSiteResidue]:
    """Predict active-site residues from the closest experimental templates."""
    candidates = [h for h in hits if "pdb" in (h.database or "").lower() and h.q_aln and h.t_aln]
    candidates.sort(key=lambda h: (h.rmsd is None, h.rmsd if h.rmsd is not None else float("inf"), -(h.tm_score if h.tm_score is not None else -1.0), h.e_value if h.e_value is not None else float("inf")))
    ...
    for hit in candidates:
        ...
        if not target_structure:
            continue
        for resnum, insertion, resname in _map_binding_site_to_query(hit, target_structure, query_pdb_text, query_chain_id):
            key = (resnum, insertion)
            supporters = votes.setdefault(key, [])
            if hit.target_id not in supporters:
                supporters.append(hit.target_id)
            names[key] = resname
    results = [ActiveSiteResidue(resnum, names.get((resnum, insertion)), len(supporters), supporters, insertion) for (resnum, insertion), supporters in votes.items()]
